...
被平台方占住。真空在 **Claude 原生语音**、**跨 Agent 控制**、**零配置**。Vibird 正对着这三个真空。完整定位
与竞品分析见 [`docs/human/zh/design.md`](docs/human/zh/design.md)。

## 架构

```
 设备(Rust / esp-hal)          Host bridge(Rust)              AI Agent
 长按说话 · Liz 表情动画  ──WiFi/WebSocket──▶  WS 服务端 · ASR · 集成  ──hooks/MCP──▶  Claude Code
       S3R 参考机                            cargo install · pip install · Claude 插件
```
...
Vibird 采用 **双授权**(与 [Ultralytics](https://github.com/ultralytics/ultralytics) 同一模式):
